...
SSTableIteratorMinimal::SSTableIteratorMinimal(char* data, const uint64_t data_size,
...
SSTableIteratorMinimal::~SSTableIteratorMinimal() { free(data_); };

void SSTableIteratorMinimal::Seek(const Slice& target) {
...
void SSTableIteratorMinimal::SeekForPrev(const Slice& target) {
  Seek(target);
  Prev();
}

void SSTableIteratorMinimal::SeekToFirst() {
  SeekToRestartPoint(0);
  ParseNextKey();
}

void SSTableIteratorMinimal::SeekToLast() {
  SeekToRestartPoint(count_ - 1);
  ParseNextKey();
}

void SSTableIteratorMinimal::Next() {
  assert(Valid());
  ParseNextKey();
}

// Avoid using prev!
void SSTableIteratorMinimal::Prev() {
  assert(Valid());
  SeekToRestartPoint(index_ - 1);
  ParseNextKey();
}

bool SSTableIteratorMinimal::ParseNextKey() {
...
void SSTableIteratorMinimal::SeekToRestartPoint(const uint64_t index) {
  current_key_.clear();
  current_val_.clear();
  index_ = index;
...
uint64_t SSTableIteratorMinimal::GetRestartPoint(const uint64_t index) const {
