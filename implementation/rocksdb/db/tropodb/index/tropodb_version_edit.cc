...
TropoVersionEdit::TropoVersionEdit() { Clear(); }

void TropoVersionEdit::Clear() {
  last_sequence_ = 0;
...
void TropoVersionEdit::EncodeTo(std::string* dst) const {
#ifdef VERSION_LEAK
  ...
  // comparator
  if (has_comparator_) {
    PutVarint32(dst, static_cast<uint32_t>(TropoVersionTag::kComparator));
    PutLengthPrefixedSlice(dst, comparator_);
  }
  // last sequence
  if (has_last_sequence_) {
    PutVarint32(dst, static_cast<uint32_t>(TropoVersionTag::kLastSequence));
  ...
  TROPO_LOG_DEBUG("DEBUG LEAK begin data %lu \n", debug_version_leak_);
#endif

  // compaction pointers
  for (size_t i = 0; i < compact_pointers_.size(); i++) {
    PutVarint32(dst, static_cast<uint32_t>(TropoVersionTag::kCompactPointer));
  ...
  TROPO_LOG_DEBUG("DEBUG LEAK compaction pointers %lu \n", debug_version_leak_);
#endif

  // deleted range
  if (has_deleted_range_) {
    PutVarint32(dst, static_cast<uint32_t>(TropoVersionTag::kDeletedRange));
  ...
  for (auto del : deleted_ss_pers_) {
    ...
    PutVarint32(dst, static_cast<uint32_t>(TropoVersionTag::kDeletedSSTable));
...
#ifdef VERSION_LEAK
  debug_version_leak_ = dst->size() - debug_version_leak_;
  TROPO_LOG_DEBUG("DEBUG LEAK deleted LN %lu \n", debug_version_leak_);
#endif
...
#endif
    PutVarint32(dst, static_cast<uint32_t>(TropoVersionTag::kNewSSTable));
    PutFixed8(dst, level);  // level
    PutVarint64(dst, m.number);
...
#ifdef VERSION_LEAK
  debug_version_leak_ = dst->size() - debug_version_leak_;
  TROPO_LOG_DEBUG("DEBUG LEAK new files %lu \n", debug_version_leak_);
#endif

  // Fragmented logs
  if (has_fragmented_data_) {
    PutVarint32(dst, static_cast<uint32_t>(TropoVersionTag::kFragmentedData));
    PutLengthPrefixedSlice(dst, Slice(fragmented_data_));
  }

#ifdef VERSION_LEAK
  debug_version_leak_ = dst->size() - debug_version_leak_;
  TROPO_LOG_DEBUG("DEBUG LEAK fragmented logs %lu \n", debug_version_leak_);
#endif
}
...
static bool GetLevel(Slice* input, uint8_t* level) {
  uint8_t v;
  if (GetFixed8(input, &v) && v < TropoDBConfig::level_count) {
    *level = v;
    return true;
  } else {
    return false;
...
Status TropoVersionEdit::DecodeFrom(const Slice& src) {
  const char* msg = nullptr;
  Slice input = Slice(src);
  uint32_t tag;
  TropoVersionTag versiontag;
  Slice str;
  uint8_t level;
  uint64_t number;
  ...
  while (msg == nullptr && GetVarint32(&input, &tag)) {
    versiontag = static_cast<TropoVersionTag>(tag);
    switch (versiontag) {
      case TropoVersionTag::kComparator:
        if (GetLengthPrefixedSlice(&input, &str)) {
        ...
        break;
      case TropoVersionTag::kLastSequence:
        ...
        break;
      case TropoVersionTag::kNextSSTableNumber:
        ...
        break;
      case TropoVersionTag::kDeletedRange:
        ...
        break;
      case TropoVersionTag::kDeletedSSTable:
        ...
        break;
      case TropoVersionTag::kNewSSTable:
        if (GetLevel(&input, &level) && DecodeLevel(&input, level, &m)) {
        ...
        break;
      case TropoVersionTag::kCompactPointer:
        ...
        break;
      case TropoVersionTag::kFragmentedData:
        frag.clear();
