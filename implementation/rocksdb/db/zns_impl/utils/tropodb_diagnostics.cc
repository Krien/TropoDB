...
namespace ROCKSDB_NAMESPACE {
  TimingCounter::TimingCounter() : num_(0), sum_(0), sum_squared_(0), min_(std::numeric_limits<uint64_t>::max()), max_(0) {}
  ...
  uint64_t TimingCounter::GetNum() const {
    return num_;
  }

  double TimingCounter::GetSum() const {
